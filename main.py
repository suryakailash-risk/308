...
DB_USERNAME = st.secrets.db_credentials['username']
KEY = st.secrets.db_credentials['password']
client = MongoClient(f'mongodb+srv://{DB_USERNAME}:{KEY}@cluster0.jyel19j.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0')
db = client.Login
collection_user = db.Split
...
def login():
    ...
    if st.button("Log in"):
        if username == st.secrets["passwords"].get(username) and hmac.compare_digest(password, st.secrets.passwords.get(username)):
            tempusername = username
            st.session_state["username"] = tempusername
            st.session_state["password_correct"] = True
            return True
        else:
            st.error("😕 User not known or password incorrect")
...
# Function to add split
def add_split():
    res = [user['name'] for user in db.Users.find()]
    item_name = st.text_input('Shop')
    item_cost = st.number_input('Item Cost', min_value=0)
    date_shopping = st.date_input("Date of shopping", value=datetime.date.today())
    datetime_obj = datetime.datetime.combine(date_shopping, datetime.time.min)

    num_field_pairs = st.number_input("Number of people splitting the cost", min_value=1, value=1)
    text_inputs = [st.selectbox(f'Who has to pay {i}', res) for i in range(1, num_field_pairs + 1)]
    number_inputs = [st.number_input(f'Amount for {text_inputs[i]}', min_value=0) for i in range(num_field_pairs)]

    paid_by = st.selectbox('Who paid', res)
    if st.button('Add Item'):
        for i in range(len(text_inputs)):
            collection_user.insert_one({
                ...
                'date': datetime_obj,
                'name': text_inputs[i],
                'cost': number_inputs[i],
                'payto': paid_by,
                'paid': False
            })
        st.success('Item added successfully!')

# Function to show owed transactions
def show_owed_transactions():
    option = st.selectbox("Select", ("Yet to get Payment", "Have already paid"), placeholder="Select contact method...")
    paid_status = False if option == "Yet to get Payment" else True
    users_details = collection_user.find({"payto": st.session_state["username"], "paid": paid_status})
    users_details = pd.DataFrame(list(users_details))
    temp = 0
    for index, item in users_details.iterrows():
        col1, col2, col3 = st.columns(3)
        with col1:
            st.write(item['shop'])
        with col2:
        ...
        if st.button(f"Submit Update Item {temp}"):
            ...
            newvalues = {"$set": {"paid": agree, "updated": datetime.datetime.now()}}
            collection_user.update_one(myquery, newvalues)
        temp += 1

# Function to add person details
def add_person():
    if st.session_state["username"] == "Surya":
        person_id = st.text_input('Enter Person Name')
        person_username = st.text_input('Enter Person Username')
        person_phone = st.text_input('Enter Person Phone')
        person_email = st.text_input('Enter Person Email')
...
# Function to show owed transactions by the user
def show_i_owe_you():
    users = collection_user.find({"name": st.session_state["username"], "paid": False})
    for item in users:
        st.write(item['shop'], item['name'], item['cost'])
...
def show_my_budget():
    ...
    end_date = st.date_input('End date')
    end_date = datetime.datetime.combine(end_date, datetime.time.min)
    myquery = {"name": st.session_state["username"], "date": {"$gte": start_date, "$lte": end_date}}
    if st.button("Search"):
        users = collection_user.find(myquery)
        df = pd.DataFrame(list(users))
        del df["_id"]
        del df["name"]
        del df["totalcost"]
